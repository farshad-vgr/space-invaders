...
// Building objects from the Sound class with a proper sound source for different parts of the game
const backgroundSound = new Sound("assets/sounds/bgSound.wav");
const startSound = new Sound("assets/sounds/start.mp3");
const invadersMoveSound = new Sound("assets/sounds/invadersMove.mp3");
const shootSound = new Sound("assets/sounds/shoot.wav");
const explodeSound = new Sound("assets/sounds/explode.wav");
...
let alienPositionsLength = alienPositions.length;
let alienRemovedPositions = []; // Alien remove positions will push to this array

// Fill the gameboard with divs and the total number of divs is 15*15
...
tutorialBtn.addEventListener("click", () => {
  ...
  function eventListenerAdder(btn, time) {
  ...
  eventListenerAdder(easyBtn, 1000); // Set a listener for the easy level button
  eventListenerAdder(normalBtn, 600); // Set a listener for the normal level button
  eventListenerAdder(hardBtn, 300); // Set a listener for the hard level button
...
function startGame(level) {
  ...
  function moveAliens() {
    ...
    // Change in movement direction(from left to right) depends on the alien's position
    if (isOnLeftEdge && !isGoingRight) {
      invadersMoveSound.playSound();
